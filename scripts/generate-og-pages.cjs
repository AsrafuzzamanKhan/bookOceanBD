// Generates a static HTML snapshot per book at dist/book/<name>/<id>/index.html
// with that book's real Open Graph / Twitter Card meta tags baked directly
// into the initial HTML.
//
// Why this exists: this app is a client-rendered SPA. Link-preview crawlers
// (Facebook, WhatsApp, Messenger) do NOT execute JavaScript, so they only
// ever see whatever raw HTML Firebase Hosting returns for a given URL - the
// per-page <title>/meta tags react-helmet injects in BookDetails.jsx are
// invisible to them. Firebase Hosting serves an exact static file match
// before falling back to the SPA rewrite, so placing a real file at the
// exact book URL path fixes this without needing Cloud Functions (which
// would require upgrading off the free Spark plan).
//
...
// visitors get the exact same working SPA; React Router boots normally and
// renders the actual BookDetails page. Only what a crawler sees differs.
//
